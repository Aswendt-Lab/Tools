...
def plot_citations_per_year(df_year: pd.DataFrame, title_stub: str, outdir: str = OUTDIR):
    ...
    # Simple exponential trend if enough data
    if len(years) >= 4:
        try:
            x = years.astype(float)
            y = counts.astype(float)
            y_nonzero = np.where(y <= 0, 1e-3, y)
            coeffs = np.polyfit(x, np.log(y_nonzero), 1)
            a = math.exp(coeffs[1])
            b = coeffs[0]
            xfit = np.linspace(x.min(), x.max(), 200)
            yfit = a * np.exp(b * xfit)
            ax.plot(xfit, yfit, "-", linewidth=2, label="exp. trend", color=COLOR_PALETTE[1])
        except Exception as e:
            print(f"⚠️ Could not fit exponential trend: {e}")

    ax.set_xlabel("Year")
    ax.set_ylabel("Number of citations")
    ax.set_title("Citations per year")
    ax.set_xlim(min_year, max(years) + 1)
...
def plot_citations_by_country(df_country: pd.DataFrame, title_stub: str, outdir: str = OUTDIR):
    ...
    if len(df) > 10:
        top = df.iloc[:9]
        others = df.iloc[9:]["count"].sum()
        others_row = pd.DataFrame([{"country": "Other", "count": others}])
    ...
    ax.set_title("Citations by country", fontsize=10)

    fname = os.path.join(outdir, f"{safe_filename(title_stub)}_citations_by_country_pie.png")
    fig.tight_layout()
    fig.savefig(fname, dpi=150)
    plt.close(fig)
    return fname
...
def create_pdf_report(author_name, orcid, summary_df, agg_year_plot, agg_country_plot):
    """
    Create a styled PDF report with clickable DOI links and embedded graphs.
    This function is where the previous <i> tag / </para> parse error happened.
    We now clean and escape all titles.
    """
    ...
    if summary_df is not None and not summary_df.empty:
        for _, row in summary_df.iterrows():
            ...
            year_plot = f"{base}_citations_per_year.png"
            country_plot = f"{base}_citations_by_country_pie.png"

            # --- SAFE HEADING: strip HTML tags then escape ---
            title_text_no_tags = re.sub(r"<[^>]+>", "", title)
            truncated = title_text_no_tags[:90]
            if len(title_text_no_tags) > 90:
                truncated += "..."
            title_safe = safe_para(truncated)

            elements.append(
                Paragraph(f"<b>Publication:</b> {title_safe}", styles['Heading3'])
            )
            ...
            if os.path.exists(country_plot):
...
def main():
    ...
    # Aggregate plots across all selected works
    ...
    # Final PDF report
    create_pdf_report(author_name, author_orcid, df_summary, agg_year_plot, agg_country_plot)
    print("\n✅ Done.")
