...
def crop_video(video_path, crop_box, mirror=False):
    cap = cv2.VideoCapture(video_path)
    ...
    while True:
        ...
        if mirror:
            ...
            frame = frame[int(crop_box[1]):int(crop_box[1] + crop_box[3]), int(crop_box[0]):int(crop_box[0] + crop_box[2])]
        out.write(frame)
    cap.release()
    out.release()
...
def process_multiple_videos():
    ...
    mirror = input("Do you want to mirror the videos before cropping? (y/n): ").lower() == "y"
    new_fps = input("Enter the new FPS for the videos (press Enter to keep original FPS): ")
    for video_path in video_paths:
        save_path, old_fps = crop_video(video_path, crop_box, mirror)
        if new_fps:
