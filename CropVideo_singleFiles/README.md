...
- `CropVideo_SetFramrate_GUI_v1.py` and `v2.py` are earlier GUI variants.
- `CropVideo_SetFramrate_GUI_v3.py` supports single-video or whole-folder processing and is the newest version here.

Requirements include Python with Tkinter, OpenCV, `ffmpeg`, and—specifically for `CropVideo_SetFramrate.py`—`avprobe`.

```bash
python CropVideo_SetFramrate_GUI_v3.py
```
